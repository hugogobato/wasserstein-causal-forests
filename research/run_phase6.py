#!/usr/bin/env python3
"""Launcher for the Phase 6 track (mechanisms + income realism).

    python research/run_phase6.py freeze
    python research/run_phase6.py run --workers 10 [--track a|b] [--limit N]
    python research/run_phase6.py merge

Same discipline as `research/run_g3.py`: thread-pinning environment variables
are set before anything imports NumPy. Cells resume by key from the per-worker
execution logs; a failed cell is recorded as failed and never retried under a
different seed.
"""
...
from wasserstein_causal_forests.g3.phase6 import (  # noqa: E402
    PHASE6_CONTRACT_ID,
...
MANIFEST_PATH = ROOT / "results" / "manifests" / "phase6_manifest.json"
EXECUTION_LOG = ROOT / "results" / "manifests" / "phase6_execution_log.jsonl"
RESULTS_DIRECTORY = ROOT / "results" / "phase6"
CACHE_DIRECTORY = ROOT / "results" / "rcpp_cache"
MERGED_PATH = ROOT / "results" / "merged_phase6" / "phase6_results.parquet"
...
def _worker(payload: tuple[list[dict], int]) -> dict:
    ...
    tag = os.environ.get("PHASE6_SHARD_TAG", "")
...
def freeze(_: argparse.Namespace) -> int:
    ...
    document = build_phase6_manifest()
    MANIFEST_PATH.write_text(json.dumps(document, indent=2), encoding="utf-8")
    try:
        import yaml

        (ROOT / "configs" / "simulation_phase6.yaml").write_text(
...
def run(arguments: argparse.Namespace) -> int:
    ...
    wanted_grids = {"a": {"main"}, "b": {"income"}}
    if arguments.track:
        keep = wanted_grids[arguments.track]
...
def merge(_: argparse.Namespace) -> int:
    ...
    shard_paths = sorted(
        ...
        + list((RESULTS_DIRECTORY / "trackA_shards").glob("shard_*.parquet"))
        + list((RESULTS_DIRECTORY / "trackB_shards").glob("shard_*.parquet"))
    )
    for path in shard_paths:
        frame = pq.read_table(path).to_pandas()
        duplicates = seen & set(frame["cell_key"])
        if duplicates:
            raise SystemExit(f"duplicate cell keys across shards: {sorted(duplicates)[:4]}")
...
def main() -> int:
    ...
    run_parser.add_argument("--track", choices=("a", "b"), default="")
