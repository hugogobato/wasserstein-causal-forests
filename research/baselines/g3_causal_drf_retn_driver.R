#!/usr/bin/env Rscript
#
# G3 Phase 6.5 driver: Causal-DRF at an explicit kernel bandwidth multiplier.
#
# Identical to g3_causal_drf_original_driver.R except for one argument: the
# Gaussian kernel bandwidth is set to
#
#     bandwidth = spec$bandwidth_multiplier * drf:::medianHeuristic(Q_train)
#
# instead of being left to the package's data-driven default. Everything else,
# including the fit call's remaining arguments and the prediction path,
# follows code/causal_drf_paper-main/R/simulation_study_setup.R exactly. The
# multiplier is recorded in result.json so every row can show which value ran.
...
if (length(arguments) < 1L) {
  stop("usage: g3_causal_drf_retn_driver.R <workspace>")
}
...
if (is.null(spec$bandwidth_multiplier)) {
  stop("g3_causal_drf_retn_driver.R requires spec$bandwidth_multiplier")
}
bandwidth_multiplier <- as.numeric(spec$bandwidth_multiplier)
if (!is.finite(bandwidth_multiplier) || bandwidth_multiplier <= 0) {
  stop("bandwidth_multiplier must be finite and positive")
}
default_bandwidth <- drf:::medianHeuristic(Q_train)
fit_bandwidth <- bandwidth_multiplier * default_bandwidth
...
write_json(
  list(
    ...
    driver = "g3_causal_drf_retn_driver.R",
    bandwidth_multiplier = bandwidth_multiplier,
    default_bandwidth = unname(default_bandwidth),
    fit_bandwidth = unname(fit_bandwidth),
